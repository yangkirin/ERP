...
public class ProductionOrderController extends AbstractController {
    ...
    public R reportSearch(@RequestParam Map<String, Object> params) {
...
//					Map<String,Object> tempMap = map.get(key);
    ...
    @Autowired
    OutportInfoService outportInfoService;
    ...
    @RequestMapping("/back")
//	@RequiresPermissions("sales:productionorder:save")
    public R back(@RequestParam("orderId") Long orderId) {
        //根据订单ID，查询订单是否产生已确认的出库单，如存在已确认的出库单，则不允许反确认
        boolean isBack = true;
        Map<String, Object> params = new HashMap<>();
        params.put("orderId", orderId);
        List<OutportInfoEntity> outportInfoEntityList = outportInfoService.queryList(params);

        if (outportInfoEntityList != null && outportInfoEntityList.size() > 0) {
            isBack = false;
        }
        Map<String, Object> map = new HashMap<>();

        map.put("isBack", isBack);
        return R.ok().put("isBack", isBack);
    }
...
//	@RequiresPermissions("sales:productionorder:save")
    @ResponseBody
    public R upload(@RequestParam("file") MultipartFile file,@RequestParam("orderId")String orderId) throws Exception {
...
//            notNull = true;
        ...
        ProductionOrderEntity productionOrderEntity = productionOrderService.queryObject(Long.valueOf(orderId));

        //先删除已有的所有产品
        productionOrderDetailService.deleteByOrderId(Long.valueOf(orderId));
        ...
        for (int r = 1; r <= sheet.getLastRowNum(); r++) {
            ...
            params.clear();
            String prdCode = formatter.formatCellValue(row.getCell(0));
            String orderNum = formatter.formatCellValue(row.getCell(1));
            params.put("prdCode",prdCode);
            PrdDataEntity prdDataEntity = prdDataService.queryObjectMap(params);
            params.clear();
            params.put("customerId",productionOrderEntity.getCustomerId());
            params.put("prdId",prdDataEntity.getId());
            CustomerPrdEntity customerPrdEntity = customerPrdService.queryObjectMap(params);

            System.out.println("prdCode=" + prdCode + "---orderNum=" + orderNum);

            ProductionOrderDetailEntity productionOrderDetailEntity = new ProductionOrderDetailEntity();
            productionOrderDetailEntity.setProductionOrderId(Long.valueOf(orderId));
            productionOrderDetailEntity.setPrdId(prdDataEntity.getId());
            productionOrderDetailEntity.setPrdNo(prdDataEntity.getPrdCode());
            productionOrderDetailEntity.setPrdName(prdDataEntity.getPrdName());
            productionOrderDetailEntity.setPrdTypeId(Long.valueOf(prdDataEntity.getTypeId()));
            productionOrderDetailEntity.setPrdTypeName(prdDataEntity.getTypeName());
            productionOrderDetailEntity.setProductionOrderNo(productionOrderEntity.getProductionNo());
            productionOrderDetailEntity.setCost(new BigDecimal(prdDataEntity.getCost() == null ? "0" : prdDataEntity.getCost().toString()));

            productionOrderDetailEntity.setPrice1(prdDataEntity.getReferencePrice());
            if(customerPrdEntity != null && customerPrdEntity.getPrdPrice() != null){
                productionOrderDetailEntity.setPrice2(customerPrdEntity.getPrdPrice().toString());
            }else{
                productionOrderDetailEntity.setPrice2(prdDataEntity.getReferencePrice());
            }

            productionOrderDetailEntity.setAmount(orderNum);

            //计算预估收入=(数量*售价）-（数量*定价）
            BigDecimal revenue = new BigDecimal(0);
            revenue = new BigDecimal(productionOrderDetailEntity.getAmount()).multiply(new BigDecimal(productionOrderDetailEntity.getPrice2()));
//            .subtract(new BigDecimal(productionOrderDetail.getAmount()).multiply(productionOrderDetail.getCost())).setScale(2, BigDecimal.ROUND_HALF_DOWN);
            productionOrderDetailEntity.setRevenue(revenue.toString());

            productionOrderDetailService.save(productionOrderDetailEntity);
        }

        return R.ok().put("code","00");
    }
