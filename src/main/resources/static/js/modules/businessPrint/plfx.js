$(function () {
    ...
    $("#jqGrid").jqGrid({
        ...
        colModel: [
            ...
            {label: '单锅量', name: 'potWeight', index: 'potWeight', width: 80},
            {label: '锅数', name: 'potCount', index: 'potCount', width: 80},
            {label: '不足锅量', name: 'residueCount', index: 'residueCount', width: 80,formatter:function(value, options, row){
                if(row.miniRate == null || row.miniRate == 0){
                    return value;
                }
                return Number(value/row.miniRate).toFixed(4);
...
var vm = new Vue({
    ...
    methods: {
        ...
        print:function(){
            ...
            openPostWindow(baseURL + "businessPrint/biSearch/PrintPLFXNew?token=" + token+"&takeStn="+vm.takeStn+"&demandDate="+vm.productionOrder.demandDate,JSON.stringify(printData),'printPLFX');
        }
    }
});
var setting = {
